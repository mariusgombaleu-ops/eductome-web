...
import { Menu, X, BookOpen, FileText, Info, Edit3, HeartHandshake, Download, Share, PlusSquare, ArrowRight } from 'lucide-react';
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isTransparent ? 'bg-transparent py-4' : 'bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-sm py-0'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16 md:h-20 transition-all duration-300">
            {/* Logo — épuré (ombre portée, pas de boîte) */}
            <Link to="/" className="flex flex-shrink-0 items-center py-2" onClick={closeMenu} aria-label="Accueil EDUCTOME">
              <img
                src="/images/logo.png?v=2"
                alt="Eductome Logo"
                className={`transition-all duration-300 object-contain ${isTransparent ? 'h-12 md:h-14 bg-white/95 p-1.5 rounded-xl shadow-lg backdrop-blur-sm' : 'h-11 md:h-14'}`}
              />
            </Link>
            ...
            <div className="hidden md:flex items-center gap-7">
                  ...
                  <Link
                    ...
                    className={`font-semibold text-sm transition-all duration-200 border-b-2 py-1 ${
                      isActive
                        ...
                        : isTransparent
                          ? 'text-white/90 border-transparent hover:text-white hover:border-white/40 drop-shadow-md'
              ...
              <div className="flex items-center gap-3 pl-3">
                {isLoggedIn ? (
                  <Link to="/dashboard" className={`${primaryCta} px-5 py-2.5 text-sm`}>
                  ...
                  <>
                    <Link
                      to="/login"
                      className={`font-semibold text-sm transition-colors ${isTransparent ? 'text-white/90 hover:text-white drop-shadow-md' : 'text-gray-700 hover:text-eductome-magenta'}`}
                    >
                      Se connecter
                    </Link>
                    <Link to="/register" className={`${primaryCta} px-5 py-2.5 text-sm`}>
            ...
            <div className="md:hidden flex items-center gap-2">
              {!isOpen && (
                <Link
                  ...
                  className={`${primaryCta} px-3.5 py-2 text-[13px]`}
                >
              ...
              <button
                onClick={toggleMenu}
                className={`p-2 rounded-xl focus:outline-none transition-colors relative z-50 ${
                  isTransparent
        ...
        {(isOpen || isClosing) && (
          <div className="md:hidden fixed inset-0 top-[60px] md:top-[80px] bg-black/60 backdrop-blur-sm z-40 animate-in fade-in" onClick={closeMenu}>
            <div
              className={`absolute top-4 left-4 right-4 bg-white/95 dark:bg-[#161B22]/95 backdrop-blur-xl border border-white/20 dark:border-[#30363D] shadow-2xl rounded-3xl p-5 transition-all duration-300 ease-in-out transform origin-top ${isClosing ? 'scale-95 opacity-0 -translate-y-4' : 'scale-100 opacity-100 translate-y-0 animate-in slide-in-from-top-4'}`}
              onClick={e => e.stopPropagation()}
            >
              {/* Le menu mène d'abord au geste de conversion */}
              <div className="flex flex-col gap-2 mb-3">
                {isLoggedIn ? (
                  <Link to="/dashboard" onClick={closeMenu} className={`${primaryCta} w-full py-3.5 text-base`}>
                    Mon Espace <ArrowRight className="w-4 h-4" />
                  </Link>
                ) : (
                  <>
                    <Link to="/register" onClick={closeMenu} className={`${primaryCta} w-full py-3.5 text-base`}>
                      Commencer gratuitement <ArrowRight className="w-4 h-4" />
                    </Link>
                    <Link to="/login" onClick={closeMenu} className="w-full text-center py-2 text-sm font-semibold text-eductome-marine dark:text-white hover:text-eductome-magenta transition-colors">
              ...
              <div className="flex flex-col space-y-1.5 mt-2">
                {navLinks.map((link) => {
                  ...
                  return (
                    <Link
                      ...
                      to={link.path}
                      onClick={closeMenu}
                      className={`font-semibold text-base flex items-center gap-4 p-3 rounded-2xl transition-all ${
                        isActive
                          ? 'bg-gradient-to-r from-pink-50 to-white dark:from-[#D81B60]/20 dark:to-transparent text-eductome-magenta border border-pink-100 dark:border-pink-900/30'
                ...
                {isInstallable && (
                  <div className="pt-3 mt-1 border-t border-gray-100 dark:border-[#30363D]">
                    <button
                      onClick={() => {
                        closeMenu();
                        installPWA();
                      }}
                      className="w-full bg-eductome-marine text-white px-4 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-[#15294a] transition-all active:scale-95"
                    >
                      <Download className="w-5 h-5" /> Installer l'app sur mon téléphone
                    </button>
                  </div>
                )}
              </div>
