...
import { BookOpen, Target, Calendar, Circle } from 'lucide-react';
...
export const NewUserWelcome = ({ hasTimetable, hasReadFirstCourse }: NewUserWelcomeProps) => {
  ...
  // If all tasks are completed, hide the component entirely
  if (isGoalsSet && hasTimetable && hasReadFirstCourse) {
    return null;
  }
        ...
        <div className="p-4 md:p-5 space-y-3">
          ...
          {!hasReadFirstCourse && (
            ...
            >
              ...
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-[#9CA3AF] group-hover:text-[#D81B60] transition-colors">
          ...
          {!isGoalsSet && (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: '#E6510015', color: '#E65100' }}>
                  <Target className="w-5 h-5" />
                </div>
                <div className="text-left">
                  <p className="text-sm font-bold" style={{ color: palette.ink }}>Définir mes objectifs chiffrés</p>
                  <p className="text-xs font-medium" style={{ color: palette.ink3 }}>Combien de points au {examName} ?</p>
                </div>
              </div>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors text-[#9CA3AF] group-hover:text-[#E65100]`}>
                <Circle className="w-5 h-5" />
              </div>
            </button>
          )}
          ...
          {!hasTimetable && (
            ...
            >
              ...
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-[#9CA3AF] group-hover:text-[#1976D2] transition-colors">
                <Circle className="w-5 h-5" />
              </div>
            </button>
          )}
